...
GROUP BY
    ...
    */
    ...
    -----------------------8.BÖLÜM-------------------------
-- ÖRNEK SORGU: BİR KULLANICININ YAPTIĞI TÜM SİPARİŞLERİN DETAYLARINI GETİRMEK İSTİYORUZ.
-- BUNUN İÇİN USER_ ,BASKET ,BASKETDETAIL ,ITEM ,ORDER_ ,ADDRES ,CITY ,TOWN ,DISTRICT TABLOLARINI KULLANACAĞIZ.
-- VE BU TABLOLARI BİRLEŞTİRECEĞİZ.

    SELECT U.NAMESURNAME,B.CREATEDDATE ,BD.AMOUNT,BD.PRICE,BD.TOTAL ,
I.ITEMCODE,I.ITEMNAME,I.CATEGORY1,I.CATEGORY2,I.CATEGORY3
FROM USER_ AS U
JOIN BASKET AS B ON B.USERID=U.ID
JOIN BASKETDETAIL AS BD ON BD.BASKETID=B.ID
JOIN ITEM AS I ON I.ID=BD.ID
JOIN ORDER_ AS O ON O.BASKETID=B.ID
JOIN ADDRES AS A ON A.ID=O.ADDRESSID
JOIN CITY AS C ON C.ID=CITYID
JOIN TOWN AS T ON T.ID=TOWNID
JOIN DISTRICT AS D ON D.ID=DISTRICTID
ORDER BY
U.NAMESURNAME ,U.CREATEDDATE
